...
#              clus.size = floor(runif(nstudies, 400, 400)), eta2 = runif(ncoef, .95, 1),
...
sim_data <- function(nstudies, ncoef, ntest){
  ...
  for (i in 1:nstudies){
    ...
    #For a cubic outcome:
    y <- as.matrix((data_i[,vars]) %*% curcoefs) +
      icoefs[1]*(data_i[,vars[1]])^2 - icoefs[2]*(data_i[,vars[1]])^3 + 
      cbind(rnorm(nrow(data_i))) # Added noise
    #new_y <- ifelse(y >= quantile(y, runif(1, .5, .9)), 1, 0)
    
    #For a binary outcome:
    
    #bin_cutoff <- c(.65, runif(ncol(data_i) - 1, .3, .7))
    #bin_cutoff <- c(.65, .4)
    #y <- as.matrix((sapply(1:nchoose, function(x) {ifelse(data_i[,x] <= quantile(data_i[,x], bin_cutoff[x]), 0, 1)})) %*% curcoefs) +
    # + icoefs[1]*data_i[,vars[1]]*data_i[,vars[2]]
    #- icoefs[2]*data_i[,vars[1]]*data_i[,vars[3]] +
    # cbind(rnorm(nrow(data_i))) # Added noise
    
    #y <- coefs * sin(ifelse(data_i[,1] <= quantile(data_i[,1], bin_cutoff[1]), 0, 1)) + cbind(rnorm(nrow(data_i)))
...
clusters_fit <- function(modfit, modpred, ndat, ncoef, ntest, studies_list, cluster_ind, nsep){
  ...
  for(i in (ntrain + 1):(length(edat))){
    ...
    preds_mat[[i - ntrain]] <- cbind(cury, merged, stack_ridge)
  ...
  return(list(outmat = outmat, preds_mat = preds_mat))
}


sd <- sim_data(nstudies = 10, ncoef = 20, ntest = 5)$studies_list
cf <- clusters_fit(modfit = randomforestfit, modpred = randomforestpredict, ndat = 10, 
                   ncoef = 20, ntest = 5, studies_list = sd, cluster_ind  = 1, nsep = 30)


####################################################
####################################################
#Plot
preds_mat <- cf$preds_mat[[5]]
cury <-  preds_mat[,1]
merged <- preds_mat[,2]
stack_ridge <- preds_mat[,3]

RMSE(cury, stack_ridge)
RMSE(cury, merged)
...
#Binary
#bin_cutoff <- c(.65, .4)
#true_y <- (ifelse(x1 <= quantile(x, .65), 0, 1)) *10 

#Cubic
true_y <- 5*x1 + 4*x1^2 - 1.8*x1^3

#truth
par(mfrow = c(1, 3))
plot(x, preds_mat[, 1], xlab = "x", ylab = "y", pch = 20, main = "Truth")
lines(x1, true_y)
legend(.5, 110, legend=c("Truth"),
       col=c("black"), pch = c(20), cex= 1.2)

#merged
plot(x, preds_mat[, 1], xlab = "x", ylab = "y", pch = 20, main = "Merged")
points(x, preds_mat[, 2], col = "green", pch = 0)
lines(x1, true_y)
legend(.5, 110, legend=c("Truth", "Merged"),
       col=c("black", "green"), pch = c(20, 0), cex= 1.2)

#ensemble
plot(x, preds_mat[, 1], xlab = "x", ylab = "y", pch = 20, main = "Clustered RF")
points(x, preds_mat[, 3], col = "red", pch = 23)
lines(x1, true_y)
legend(0, 110, legend=c("Truth",  "Ensemble"),
       col=c("black", "red"), pch = c(20,  23), cex= 1.2)
