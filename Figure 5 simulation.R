...
# sim_data <- function(nstudies, ncoef, ntest, sampsize){
...
#              clus.size = floor(runif(nstudies, sampsize, (sampsize + 100))), eta2 = runif(ncoef, .5, 1),
...
sim_data <- function(nstudies, ncoef, ntest, sampsize){
  ntest = 1
  studies_list  <- trueY_list <- vector("list", nstudies)
  nchoose <- 10
  #general predictor-outcome rule:
  coefs <- sample(c(runif(round(nchoose/2), -5, -0.5), runif(nchoose - round(nchoose/2), 0.5, 5)))
  vars <- sample(1:ncoef, nchoose)
  ...
  for (i in 1:nstudies){
    ...
    y <- as.matrix((data_i[,vars]) %*% curcoefs) +
      ...
      cbind(rnorm(nrow(data_i))) # Added noise
    #new_y <- ifelse(y >= quantile(y, runif(1, .5, .9)), 1, 0)
    true_y <- as.matrix((data_i[,vars]) %*% curcoefs)
    trueY_list[[i]] <- true_y
    studies_list[[i]] <- as.data.frame(cbind(y, data_i))
    colnames(studies_list[[i]]) <- c("y", paste0("V", 1:ncoef))
  }
  return(list(studies_list = studies_list, trueY_list = trueY_list))
}

sim_bootstrap <- function(sd){
  ntest = 1
  cl <- sd$studies_list
  cli <- lapply(1:length(cl), function(i) cbind(i, cl[[i]]))
  merged <- do.call(rbind, cli[1:(length(cli) - ntest)])
  bootsample <- sample(nrow(merged), replace = TRUE)
  merged_boot <- as.data.frame(merged[bootsample, ])
  
  trueY <- sd$trueY_list
  trueYi <- lapply(1:length(trueY), function(i) cbind(i, trueY[[i]]))
  merged_trueY <- as.data.frame(do.call(rbind, trueYi[1:(length(trueYi) - ntest)]))
  trueY_boot <- merged_trueY[bootsample,]
  
  studies_list <- trueY_list <- vector("list", length(cl))
  for (j in 1:(length(trueY) - ntest)){
    studies_list[[j]] <- as.data.frame(merged_boot[,-1])[merged_boot$i == j, ]
    trueY_list[[j]] <- as.data.frame(trueY_boot[,-1])[trueY_boot$i == j, ]
  }
  studies_list[[length(studies_list)]] <- cl[[length(cl)]]
  trueY_list[[length(trueY_list)]] <- trueY[[length(trueY)]]
  return(list(studies_list = studies_list, trueY_list = trueY_list))
}
...
create_clusters <- function(studies_list, ntest, ncoef){
  ...
  ss <- silhouette_score(2:(floor(nrow(merged)/50)), merged[,-1])
...
clusters_fit <- function(modfit, modpred, ndat, ncoef, ntest, sd, cluster_ind, nsep){
  studies_list <- sd$studies_list
  trueY_list <- sd$trueY_list
  ...
  outmat <- matrix(NA, nrow(edat[[length(edat)]]), 15)
  
  colnames(outmat) <- c("Cury","Merged", "Unweighted", "Sample_Weighted", "CS_Weighted",
  ...
  for(i in (ntrain + 1):(length(edat))){
    ...
    outmat[1:length(cury),] <- cbind(cury, merged, unweighted, sample_wtd, cs_wtd, stack_noint, stack_noint_norm, stack_int, ss_noint, ss_noint_norm, 
                                     ss_int, stack_lasso, ss_lasso, stack_ridge, ss_ridge)
    
  }
  return(list(outmat = outmat))
}

rep.clusters_bootstrap <- function(reps, modfit, modpred, ndat, ntest, ncoef, sampsize){
  sd <- sim_data(nstudies = ndat, ncoef = ncoef, ntest = ntest, sampsize = sampsize)
  cury <- as.data.frame(sd$studies_list[[length(sd$studies_list)]])$y
  merged_runs <- stackridge_runs <-matrix(NA, nrow(sd$studies_list[[length(sd$studies_list)]]), reps)
  
  for (i in 1:reps){
    sd_boot <- sim_bootstrap(sd)
    errors_multi <- as.data.frame(clusters_fit(modfit, modpred, ndat, ncoef, ntest, sd_boot, cluster_ind = 3, nsep = 0)$outmat)
    merged_runs[,i] <- errors_multi$Merged
    stackridge_runs[,i] <- errors_multi$Stack_ridge
  }
  bias2_merged <- mean((rowMeans(merged_runs) - cury)^2)
  var_merged <- mean(apply(merged_runs, 1, var))
  rmse_merged <- mean(sapply(1:reps, function(i) (merged_runs[,i] - cury)^2))
  
  bias2_stackridge <- mean((rowMeans(stackridge_runs) - cury)^2)
  var_stackridge <- mean(apply(stackridge_runs, 1, var))
  rmse_stackridge <- mean(sapply(1:reps, function(i) (stackridge_runs[,i] - cury)^2))
  
  outmat <- data.frame(matrix(NA, 1, 6))
  colnames(outmat) <- c("Merged_bias2", "Merged_var", "Merged_rmse", "Stackridge_bias2", "Stackridge_var", "Stackridge_rmse")
  outmat[1,] <- c(bias2_merged, var_merged, rmse_merged, bias2_stackridge, var_stackridge, rmse_stackridge)
  return(list(outmat = outmat))
}


rep.clusters_fit <- function(n, reps, modfit, modpred, ndat, ntest, ncoef, sampsize){
  #####
  logfile <- paste0("outputFile","nonsparsity",".txt")
  writeLines(c(""), file(logfile,'w'))
  
  num.threads <- as.integer(n/10)# round up
  ...
  results <- foreach(i = 1:n, .combine = 'rcomb', .multicombine = TRUE, .export = ls(globalenv())) %dopar% {
    library(locfit)
    library(fungible)
    library(mvtnorm)
    library(ClusterR)
    ...
    library(BiocGenerics)
    library(clusterGeneration)
    errors_multi <- rep.clusters_bootstrap(reps, modfit, modpred, ndat, ntest, ncoef, sampsize)$outmat
    print(errors_multi)
    return(list(errors_multi = errors_multi))
  }
  closeAllConnections()
  
  errors_multi = na.omit(results[[1]])
  
  colnames(errors_multi) <- c("Merged_bias2", "Merged_var", "Merged_rmse", "Stackridge_bias2", "Stackridge_var", "Stackridge_rmse")
  means_multi <- colMeans(errors_multi)
  sds_multi <- apply(errors_multi, 2, sd)
  
  
  return(list(means_multi = means_multi,
              sds_multi = sds_multi,
              errors_multi = errors_multi
  ))   
}

vary_levels <- function(n, reps, var_list, modfit, modpred, ndat, ntest, out_str){
  ptm = proc.time()
  colnames_total <- c("Merged_bias2", "Merged_var", "Merged_rmse", "Stackridge_bias2", "Stackridge_var", "Stackridge_rmse")
  total_means_multi <- array(0, c(length(var_list), 6))
  total_sds_multi <- array(0, c(length(var_list), 6))
  
  colnames(total_means_multi) <- colnames_total 
  colnames(total_sds_multi) <- colnames_total
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(n, reps, modfit, modpred, ndat, ntest, ncoef = 15, sampsize = level)
    #means
  ...
  return(list(total_means_multi = total_means_multi,
              total_sds_multi = total_sds_multi))
}

#Run the following code for simulation:
nsize_list(100, 500, 1000)
cs <- vary_levels(n = 100, reps = 100, var_list = nsize_list, modfit = randomforestfit, modpred = randomforestpredict, ndat = 6, ntest = 1, out_str = "cs")
