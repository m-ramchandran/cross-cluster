...
sim_data <- function(nstudies, ncoef, ntest, sampsize){
  ...
  m <- monte(seed = sample(1:1000, 1), nvar = ncoef, nclus = nstudies,
             clus.size = floor(runif(nstudies, sampsize, (sampsize + 10))), eta2 = runif(ncoef, .5, 1),
...
rep.clusters_fit <- function(reps, modfit, modpred, ndat, ntest, ncoef, sampsize){
  ...
  results <- foreach(i = 1:reps, .combine = 'rcomb', .multicombine = TRUE, .export = ls(globalenv())) %dopar% {
    ...
    library(BiocGenerics)
    library(clusterGeneration)
    sd <- sim_data(nstudies = ndat, ncoef = ncoef, ntest = ntest, sampsize = sampsize)$studies_list
    #for cluster: 
...
vary_levels <- function(reps, var_list, modfit, modpred, ndat, ntest, out_str){
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(reps, modfit, modpred, ndat, ntest, ncoef = 20, sampsize = level)
    print(level_rep)
    #means
...
nsize_list <- seq(100, 2500, 100)
cs <- vary_levels(reps = 250, var_list = nsize_list, modfit = randomforestfit, modpred = randomforestpredict, ndat = 10, ntest = 5, out_str = "cs")