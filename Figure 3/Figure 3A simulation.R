...
sim_data <- function(nstudies, ncoef, ntest, norm.betas){
  ...
  norm.coefs <- norm_vec(coefs)
  coefs <- (coefs/norm.coefs)*norm.betas
  #cicoefs <- icoefs/norm.coefs
...
rep.clusters_fit <- function(reps, modfit, modpred, ndat, ntest, ncoef, k, norm.betas){
  #####
  logfile <- paste0("outputFile","nonsparsity",".txt")
  writeLines(c(""), file(logfile,'w'))
  
  num.threads <- round(10)# round up
  ...
  results <- foreach(i = 1:reps, .combine = 'rcomb', .multicombine = TRUE, .export = ls(globalenv())) %dopar% {
    ...
    library(BiocGenerics)
    library(clusterGeneration)
    sd <- sim_data(nstudies = ndat, ncoef = ncoef, ntest = ntest, norm.betas = norm.betas)$studies_list
    #for cluster: 
...
vary_levels <- function(reps, var_list, modfit, modpred, ndat, ntest, out_str){
  ...
  for (i in 1:length(var_list)){
    ...
    level_rep <- rep.clusters_fit(reps, modfit, modpred, ndat, ntest, ncoef = 5, k = 80, norm.betas = level)
    #means
...
cs <- vary_levels(reps = 250, var_list = norm_list, modfit = randomforestfit, modpred = randomforestpredict, ndat = 10, ntest = 5, out_str = "cs")